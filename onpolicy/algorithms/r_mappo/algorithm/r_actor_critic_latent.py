...
class R_Actor_sigma(nn.Module):
    ...
    def forward(self, obs, rnn_states, masks, sigma, available_actions=None, deterministic=False):
        ...
        sigma_full = torch.repeat_interleave(sigma, obs.size(0) // sigma.size(0), dim=0)
        cat_obs = torch.cat([obs, sigma_full],dim=-1)

        actor_features = self.base(cat_obs)
    ...
    def get_probs(self, obs, rnn_states, sigma, max_n, masks, available_actions=None, active_masks=None):
        ...
        probs_all = self.act.get_probs(actor_features, available_actions=available_actions)
...
class R_Critic_sigma(nn.Module):
    ...
    def forward(self, cent_obs, rnn_states, masks, sigma):
        ...
        sigma_full = torch.repeat_interleave(sigma, cent_obs.size(0) // sigma.size(0), dim=0)
        cat_obs = torch.cat([cent_obs, sigma_full],dim=-1)

        critic_features = self.base(cat_obs)
        if self._use_naive_recurrent_policy or self._use_recurrent_policy:
