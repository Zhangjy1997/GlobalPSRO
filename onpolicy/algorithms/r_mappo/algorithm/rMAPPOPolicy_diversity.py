...
def kl_divergence(prob_a, prob_b, soft_mask = True):
    """Compute the PSD-PSRO diversity penalty.

    With soft_mask=True this is not the standard KL divergence; it follows the
    PSD-PSRO reference source:
    https://github.com/nigelyaoj/policy-space-diversity-psro/blob/4920c5d4d9426bc7acba0798fe8fba7b2a83ae31/poker/main.py#L51-L67
    """
...
class R_MAPPOPolicy:
    ...
    def reset_policy(self):
        self.actor = R_Actor(self.args, self.obs_space, self.act_space, self.device)
        self.critic = R_Critic(self.args, self.share_obs_space, self.device)
